...
RSpec.describe Article, type: :model do
  describe '#validations' do
    ...
    it 'should validate the presence of the published_at' do
      ...
      expect(article).not_to be_valid
      expect(article.errors.messages[:published_at]).to include("can't be blank")
    end
    ...
    it 'should validate uniqueness of the hackernews_article_id' do
      article =  create :article
      invalid_article =  build :article, slug: 'new_slug', hackernews_article_id: article.hackernews_article_id
      expect(invalid_article).not_to be_valid
    end
  end
end
